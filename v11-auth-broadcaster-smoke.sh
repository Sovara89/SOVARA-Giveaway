...
trap cleanup EXIT
cp -a "$ROOT/data/." "$TMP/"
TWITCH_CLIENT_ID=aaaaaaaaaaaaaaaaaaaaaaaaaaaaaa TWITCH_CLIENT_SECRET=bbbbbbbbbbbbbbbbbbbbbbbbbbbbbb HOST=127.0.0.1 PORT=$PORT DATA_DIR="$TMP" NODE_ENV=development node "$ROOT/server.js" >/tmp/sovara-giveaway-v11-test.log 2>&1 & PID=$!
sleep 1
...
curl -fsS -b "$COOKIE" -H 'Content-Type: application/json' -d '{}' "http://127.0.0.1:$PORT/api/admin/auth/code/start" >/tmp/admin-auth-v11.json
python3 - <<'PY'
import json,urllib.parse
...
print('OK admin OAuth: server code flow + service scopes separated')
PY
